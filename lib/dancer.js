...
module.exports.start = function(grunt, options, callback) {
  ...
  fs.readFile(options.pidFile, function(err, data){
    ...
    if(data){
      ...
      grunt.log.error(('Dancer process' + pid + ' may not be running. If it is, try killing it manually and restarting.').red);
...
module.exports.kill = function(grunt, options, callback){
  ...
  if (process._dancer_session && typeof(_currentProcess === 'undefined') && ! grunt.file.exists(options.pidFile)){
    return callback();
  }
